...
module Service
  module Failover
    ...
    SERVICES = {
      ...
      'one-gre' => { _ENABLED: 'ONEAPP_VNF_GRE_ENABLED' }
    }
    ...
    def to_task(event)
      ...
      if event[:type] != 'GROUP'
        ...
        if event[:state] == 'BACKUP'
          direction = STATE_TO_DIRECTION['BACKUP']
          ignored   = false
        elsif STATE_TO_DIRECTION[event[:state]] == STATE_TO_DIRECTION[state[:state]]
          direction = :stay
          ignored   = false
        else
